    ...
    PF_LayerDef* outputP)
{
    ...
    // エラーチェックを追加
    if (err = PF_CHECKOUT_PARAM(in_data, RGBDELAY_INPUT, red_time, in_data->time_step, in_data->time_scale, &red_param)) {
        return err;
    }
    if (err = PF_CHECKOUT_PARAM(in_data, RGBDELAY_INPUT, green_time, in_data->time_step, in_data->time_scale, &green_param)) {
        PF_CHECKIN_PARAM(in_data, &red_param);
        return err;
    }
    if (err = PF_CHECKOUT_PARAM(in_data, RGBDELAY_INPUT, blue_time, in_data->time_step, in_data->time_scale, &blue_param)) {
        PF_CHECKIN_PARAM(in_data, &red_param);
    ...
    if (PF_WORLD_IS_DEEP(outputP)) {
        ...
        for (A_long y = 0; y < outputP->height; y++) {
            ...
            for (A_long x = 0; x < outputP->width; x++) {
                // アルファは最大値
                A_u_short alpha = std::max({r_pixel[x].alpha, g_pixel[x].alpha, b_pixel[x].alpha});
        ...
        for (A_long y = 0; y < outputP->height; y++) {
            ...
            for (A_long x = 0; x < outputP->width; x++) {
                // アルファは最大値
                A_u_char alpha = std::max({r_pixel[x].alpha, g_pixel[x].alpha, b_pixel[x].alpha});
                out_pixel[x].alpha = alpha;
                // ストレートアルファ（RGB値はそのまま）
                out_pixel[x].red   = r_pixel[x].red;
                out_pixel[x].green = g_pixel[x].green;
